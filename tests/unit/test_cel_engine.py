...
def test_approval_rule_with_approval(cel_engine):
    ...
    # Grant approval
    ctx.approval_store.grant_approval("manager_signoff", "INV-100", granted_by="vp_finance")
...
def test_string_trim_and_size_rule(cel_engine):
    ...
    assert cel_engine.evaluate_rule(rule, {"reason": "Valid reason exceeding ten characters"}) is True
